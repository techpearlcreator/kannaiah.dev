...
const FrontendWorld = () => {
  return (
    <div className="portfolio-cursor-scope bg-[#fdfaf5] selection:bg-black selection:text-white min-h-screen">
      <AdaptiveCursor />
      {/* Header */}
      <header className="fixed top-0 left-0 z-50 flex w-full flex-col items-center gap-3 px-4 py-4 text-white mix-blend-difference pointer-events-none sm:h-24 sm:flex-row sm:justify-between sm:px-8 sm:py-0 md:px-16">
        <a href="#home" className="inline-flex items-baseline gap-1 font-sans text-lg font-black lowercase opacity-90 transition-opacity pointer-events-auto hover:opacity-100 sm:text-xl">
          <span>kannaiah</span>
          <span aria-hidden="true" className="mx-0.5 inline-block h-1.5 w-1.5 translate-y-[-1px] rounded-full bg-current sm:h-2 sm:w-2" />
          <span>dev</span>
        </a>
        <nav className="flex max-w-full flex-wrap items-center justify-center gap-1 text-[9px] font-bold uppercase tracking-[0.16em] pointer-events-auto sm:justify-end sm:gap-2 sm:text-[10px] sm:tracking-[0.24em] md:gap-4 md:tracking-[0.3em]">
          {[
            ['#work', 'Work'],
            ['#skills', 'Skills'],
            ['#journey', 'Journey'],
      ...
      <footer className="py-4 px-10 border-t border-black/5 flex justify-center items-center">
        <div className="text-[10px] tracking-[0.5em] uppercase font-bold text-black opacity-60 text-center">
